...
namespace Onsen
{
    ...
    /// </summary>
    public class OnsenSpawnerLevelScale : MonoBehaviour, ISpawnLevelScale
    {
        /// <summary>
        ...
        /// <summary>
        /// ウェーブが全て終了した時に発火されるイベント
        /// </summary>
        public IObservable<Unit> OnFinishedFinalWave => _finishedFinalWave;
        private Subject<Unit> _finishedFinalWave = new();
        ...
        public async UniTask StartNormalSpawn(CancellationToken token)
        {
            ...
            foreach (var waveSetting in _waveSpawnerSettings)
            {
                ...
                // 最終ウェーブであればここで処理を終了し、イベント発行
                if (_waveCount > maxWave)
                {
                    Debug.Log("FinalWaveFinish");
                    _finishedFinalWave.OnNext(Unit.Default);
                    return;
                }

                // ウェーブの変更を通知
                _waveChange.OnNext(Unit.Default);
                Debug.Log("Finish1WaveAndStartCountInterval");

                // ウェーブ間のインターバル分だけ待機する
                await UniTask.Delay(waveInterval, cancellationToken: token);
                Debug.Log("Finish1WaveAndEndCountInterval");
            }
        }
        ...
        {
            //スポナー用のトークンを作成
            var spawnerCts = new CancellationTokenSource();
            var spawnerToken = spawnerCts.Token;

            foreach (var spawnData in waveSetting.spawners)
            {
                _spawner.SpawnAsync(spawnData,spawnerToken).Forget();
            }
        ...
        /// これ温泉には使わないけどInterfaceの制限で消せません
        /// </summary>
        /// <param name="token"></param>
